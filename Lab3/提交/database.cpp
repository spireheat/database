...
int main(int argc, char* argv[])
{
	...
	ifstream ifs(argv[1]); 
 	...
 	while(getline(ifs,input_buffer1)){
		transform(input_buffer1.begin(), input_buffer1.end(), input_buffer1.begin(), ::tolower);
  		if(input_buffer1.compare(0,1,"--",0,1))
		{
  			input_buffer2.append(input_buffer1);
			input_buffer2.push_back(' ');
		}
 	}
 	...
 	for(i=0;i<input_buffer2.length();i++){
 		...
 		if(i != input_buffer2.length() - 1 && strcmp(six,"create")!=0 && strcmp(six,"select")!=0
...
void sel_select(vector <string> sel_name, vector <string> from_name, vector <string> w1_name, vector <string> w2_name, vector <string> w3_name, vector <string> gb_name)
{
	int m;
	int agg = 0;
	for(m = 0; m < sel_name.size(); m++)
	{
		if(sel_name[m].find("(") != -1)
			agg++;
	}
	if(from_name.size() == 1 && gb_name.size() == 0 && agg == 0) //select1
	{
	...
	else if(from_name.size() > 1 && gb_name.size() == 0 && agg == 0) //select2
	{
		vector <string> agg_op;
		vector <string> agg_col;
		vector <string> col_name;
	...
	else if(from_name.size() == 1) //select3
	{
		vector <string> agg_op;
		vector <string> agg_col;
		vector <string> col_name;
			...
			if(flag == 1){ //use hash table to find the same tuple in table1
	    		...
	    		while(next_equal_node)
				{
					...
					if(no_group){
                        ...
                        int group_type = -1;
                        int int_group_col_value;
                        char var_group_col_value[128] = {0};
                        if(group_tbl == 1){
                            if(c_type1[group_col_index] == 0){
                                group_type = 0;
                                string_copy(int2char_buffer,0,4,data_buffer1, tuple_offset1+c_offset1[group_col_index], tuple_offset1+c_offset1[group_col_index]+4);
                                int_group_col_value = char_to_int(int2char_buffer);
                            }
                            else{
                                group_type = 1;
                                string_copy(int2char_buffer,0,4,data_buffer1, tuple_offset1+c_offset1[group_col_index], tuple_offset1+c_offset1[group_col_index]+4);
                                ...
                                if(c_offset1[group_col_index] == last_var_col_index1){
                                    ...
                                    string_copy(int2char_buffer,0,4,data_buffer1,tuple_offset1 + c_offset1[group_col_index]+4, tuple_offset1 + c_offset1[group_col_index]+8);
                                    col_end_addr = char_to_int(int2char_buffer);
                                }
                                string_copy(var_group_col_value,0,col_end_addr-col_start_addr,data_buffer1, tuple_offset1 + col_start_addr, tuple_offset1 + col_end_addr);
                            }
                        }
                        else{
                            if(c_type2[group_col_index] == 0){
                                group_type = 0;
                                string_copy(int2char_buffer,0,4,data_buffer2, tuple_offset2+c_offset2[group_col_index], tuple_offset2 + c_offset2[group_col_index]+4);
                                int_group_col_value = char_to_int(int2char_buffer);
                            }
                            else{
                                group_type = 1;
                                ...
                                if(c_offset2[group_col_index] == last_var_col_index2){
                                    ...
                                    string_copy(int2char_buffer,0,4,data_buffer2,tuple_offset2 + c_offset2[group_col_index]+4, tuple_offset2 + c_offset2[group_col_index]+8);
                                    col_end_addr = char_to_int(int2char_buffer);
                                }
                                string_copy(var_group_col_value,0,col_end_addr-col_start_addr,data_buffer2, tuple_offset2+col_start_addr, tuple_offset2+col_end_addr);

                            }
                        }   
						insertkey2(var_group_col_value, int_group_col_value, page1, tuple_offset1, i, tuple_offset2, group_type);
					}
